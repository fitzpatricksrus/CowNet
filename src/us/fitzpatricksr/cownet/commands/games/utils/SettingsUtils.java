...
public class SettingsUtils {
    ...
    public SettingsUtils(JavaPlugin plugin, String baseName, Object destination) {
        this.plugin = plugin;
    ...
    public final int getConfigValue(String key, int def) {
        return plugin.getConfig().getInt(baseName + "." + key, def);
    }

    public final long getConfigValue(String key, long def) {
        return plugin.getConfig().getLong(baseName + "." + key, def);
    }

    public final double getConfigValue(String key, double def) {
        return plugin.getConfig().getDouble(baseName + "." + key, def);
    }

    public final boolean getConfigValue(String key, boolean def) {
        return plugin.getConfig().getBoolean(baseName + "." + key, def);
    }

    public final String getConfigValue(String key, String def) {
        return plugin.getConfig().getString(baseName + "." + key, def);
    }
    ...
    public final void updateConfigValue(String key, int value) {
        plugin.getConfig().set(baseName + "." + key, value);
    }
    ...
    public void removeConfigValue(String key) {
        plugin.getConfig().set(baseName + "." + key, null);
    }
        ...
        for (String settingName : settingNames) {
            ...
            if (value == null || value instanceof Field) {
                ...
                try {
                    ...
                    value = f.get(destination).toString();
    ...
    private boolean setAutoSettingValue(String settingName, String settingValue) {
        return setAutoSettingValue(destination, settingName, settingValue);
    }
    ...
    private void reloadAutoSettings() {
        reloadAutoSettings(destination);
    }
    ...
    private HashMap<String, Field> getAutomaticSettings() {
        return getAutomaticSettings(destination);
    }
        ...
        while (clazz != Object.class) {
            for (Method method : clazz.getDeclaredMethods()) {
                if (method.getAnnotation(Command.class) != null) {
                    ...
                    if (hasMethodPermissions(sender, method.getAnnotation(Command.class))) {
    ...
    public boolean dispatchMethod(CommandSender sender, Command cmd, String label, String[] args) {
        String[] bases = (label.equalsIgnoreCase(baseName) ? new String[]{
                "",
        ...
        for (String command : bases) {
            for (int i = args.length; i >= 0; i--) {
                ...
                Method method = signature.getMethod(destination.getClass(), senderIsPlayer);
                if ((method == null) && senderIsPlayer) {
                    method = signature.getMethod(destination.getClass(), false);
                }
                ...
                if (method != null) {
                    ...
                    try {
                        ...
                        Object result = method.invoke(destination, signature.args);
    ...
    private boolean hasMethodPermissions(CommandSender sender, Command annotation) {
        ...
        public Method getMethod(Class clazz, boolean forPlayer) {
            ...
            while (clazz != Object.class) {
                try {
                    ...
                    if (method.getReturnType().equals(boolean.class) && method.isAnnotationPresent(Command.class)) {
                        return method;
                    }
