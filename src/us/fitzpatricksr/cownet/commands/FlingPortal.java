...
public class FlingPortal extends CowNetThingy implements Listener {
	...
	private void fling(final Player player) {
		...
		final int nyan = getPlugin().getServer().getScheduler().scheduleAsyncRepeatingTask(getPlugin(), new Runnable() {
			public void run() {
	...
	private Location getPlayerDestination(Player player) {
		...
		return new Location(loc.getWorld(), rotateBytes(loc.getBlockX()) ^ seed, 300, rotateBytes(loc.getBlockZ()) ^ seed, loc.getYaw(), loc.getPitch());
	}
	...
	private Location getPlayerDestinationx(Player player) {
		...
		return new Location(loc.getWorld(), loc.getBlockX() ^ seed, 300, loc.getBlockZ() ^ seed, loc.getYaw(), loc.getPitch());
	}
}
